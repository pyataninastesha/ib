...
class BanquetReview(models.Model):
    """Отзыв на проведённый банкет (привязан к заказу-банкету)."""
    ...
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, verbose_name='Пользователь')
    order = models.OneToOneField('Order', on_delete=models.CASCADE, related_name='banquet_review', verbose_name='Заказ')
    rating = models.IntegerField(choices=RATING_CHOICES, verbose_name='Оценка')
    ...
    def __str__(self):
        return f'Банкет #{self.order_id} - {self.user} ({self.rating})'
