...
def admin_reports(request):
    ...
    # период отчёта можно менять через ?days=7|14|30|90|180|365
    ...
    try:
        ...
        days = 7
    if days not in set(days_options):
        days = 7
    ...
    # суммарные количества по закупленным позициям
    # quantity хранится строкой, поэтому аккуратно парсим первое число (поддерживаем "1", "1.5", "1,5")
    ...
    qty_map = {}
    for p in purchase_done_qs.only('title', 'quantity', 'unit'):
        q = _parse_qty(p.quantity)
        if q is None:
            continue
        unit = (p.unit or '').strip() or 'шт'
        key = (p.title, unit)
        qty_map[key] = qty_map.get(key, Decimal('0')) + q

    purchase_qty_totals = [
        {"title": t, "unit": u, "qty": qty}
        for (t, u), qty in qty_map.items()
    ]
    purchase_qty_totals.sort(key=lambda r: (-r["qty"], r["title"]))
    purchase_qty_totals = purchase_qty_totals[:20]
    ...
    # Суммарное количество по закупленным позициям (если quantity можно распарсить)
    # quantity хранится как строка, поэтому агрегируем в Python.
    qty_re = re.compile(r"^\s*([0-9]+(?:[\.,][0-9]+)?)")
    totals = {}
    for p in purchase_done_qs.only('title', 'quantity', 'unit'):
        q_raw = (p.quantity or '').strip()
        m = qty_re.match(q_raw)
        if not m:
            continue
        ...
        unit = (p.unit or '').strip() or 'шт'
        key = (p.title, unit)
        totals[key] = totals.get(key, Decimal('0')) + q

    purchase_qty_totals = [
        {'title': k[0], 'unit': k[1], 'qty': v}
        for k, v in totals.items()
    ]
    purchase_qty_totals.sort(key=lambda x: (-(x['qty']), x['title']))
    purchase_qty_totals = purchase_qty_totals[:20]
    ...
    return render(request, "core/admin_reports.html", {
        ...
        "purchase_qty_totals": purchase_qty_totals,
...
def cook_issue(request):
    ...
    if request.method == "POST":
        # Менеджер (admin) может смотреть заказы, но не должен менять их статусы.
        if getattr(request.user, 'role', '') != 'cook':
            return HttpResponseForbidden("Действие доступно только повару")
